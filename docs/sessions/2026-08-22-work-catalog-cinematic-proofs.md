...
## Decisions Made
- Ship seven proofs only: working-volumes, field-manuals, meridian, betawise-earth, energy-orb, sketchbook, inner-green.
- Hold Afterlight and Grove out of CASE_COPY, seed upsert, and `public/screenshots` — more craft time. Do not delete their source elsewhere.
...
- `src/lib/site/portfolio-proofs.ts` — seven-proof catalog data (null repo/demo, live status, hero paths).
...
- Hero JPGs at `public/screenshots/<slug>/1-hero.jpg`.
...
## Trade-offs Accepted
- Held Afterlight and Grove rather than shipping unfinished cards.
...
## Open Questions
- When Afterlight and Grove are ready, add CASE_COPY + seed rows + screenshots in a follow-up PR.
...
- [ ] Add Afterlight and Grove when craft time is done.
