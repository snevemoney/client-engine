/**
 * Pipeline run route contract tests — auth, 404, run: true/false, reason.
 */
...
const RUN_PREFIX = "pipeline-run-test-";

describe("POST /api/pipeline/run/[leadId]", () => {
  let leadId: string;
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const req = new NextRequest("http://x/api/pipeline/run/1", { method: "POST" });
  ...
  it("returns 404 when lead not found", async () => {
    const { POST } = await import("./route");
    const req = new NextRequest("http://x/api/pipeline/run/clxxxxxxxxxxxxxxxxxxxxxxxxxx", {
      method: "POST",
    });
  ...
  it("returns run: true when pipeline runs", async () => {
    ...
    const { POST } = await import("./route");
    const req = new NextRequest("http://x/api/pipeline/run/" + leadId, { method: "POST" });
    const res = await POST(req, { params: Promise.resolve({ leadId }) });
  ...
  it("returns run: false with reason when not eligible", async () => {
    const { runPipelineIfEligible } = await import("@/lib/pipeline/runPipeline");
    vi.mocked(runPipelineIfEligible).mockResolvedValue({
      run: false,
      reason: "locked",
