/**
 * GET /api/metrics/cycle-times — Cycle time metrics with range param.
...
import { fetchCycleTimeInput } from "@/lib/metrics/fetch-metrics";
...
export async function GET(req: NextRequest) {
  return withRouteTiming("GET /api/metrics/cycle-times", async () => {
    ...
    try {
      const input = await fetchCycleTimeInput(range);
      const metrics = computeCycleTimeMetrics(input);
      ...
      console.error("[metrics/cycle-times]", err);
      return jsonError("Failed to load cycle time metrics", 500);
    }
  });
}
