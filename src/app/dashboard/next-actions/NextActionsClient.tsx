...
export type NextActionsInitialData = {
  items: NextAction[];
  summary: NBASummary | null;
...
export function NextActionsClient({ initialData }: { initialData: NextActionsInitialData }) {
  ...
  const [preferences, setPreferences] = useState<Preference[]>(initialData.preferences);
  ...
  const entityType =
    scopeFilter === "review_stream"
      ? "review_stream"
      : scopeFilter === "founder_growth"
        ? "founder_growth"
        : "command_center";
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <div className="flex items-center gap-1.5">
          ...
          >
            ...
            <option value="founder_growth">Growth Pipeline</option>
          </select>
        </div>
        <select
          value={statusFilter}
                                ...
                                onClick={async () => {
                                  if (sa.confirm) {
                                    const ok = await confirm({
                                      title: sa.confirm.title,
                                      body: sa.confirm.body,
                                      variant: "destructive",
                                    });
                                    if (!ok) return;
                                  }
