...
export type DeliveryInitialData = {
...
export function DeliveryClient({ initialData }: { initialData: DeliveryInitialData }) {
  ...
  const [projects, setProjects] = useState<DeliveryProject[]>(initialData.projects);
  const [pagination, setPagination] = useState(() => normalizePagination(initialData.pagination, initialData.projects.length));
  const [summary, setSummary] = useState<Summary | null>(initialData.summary);
  const [risk, setRisk] = useState<IntelligenceContext["risk"] | null>(initialData.risk);
  const [nba, setNba] = useState<IntelligenceContext["nba"] | null>(initialData.nba);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!listRes.ok) {
        ...
        throw new Error(errData?.error ?? `Failed to load projects (${listRes.status})`);
      }
