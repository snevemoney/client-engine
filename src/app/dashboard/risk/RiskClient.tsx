...
export function RiskClient({ initialData }: { initialData: RiskInitialData }) {
  ...
  const { execute: handleRunRules, pending: runLoading } = useAsyncAction(
    async () => fetchJsonThrow("/api/risk/run-rules", { method: "POST" }),
    { toast: toastFn, successMessage: "Risk rules executed", onSuccess: () => void fetchData() },
  );

  const { execute: handleSnooze, pending: snoozePending } = useAsyncAction(
    async (id: string, preset: "2d" | "7d") =>
      fetchJsonThrow(`/api/risk/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ action: "snooze", preset }),
      }),
    { toast: toastFn, onSuccess: () => void fetchData() },
  );

  const { execute: handleResolve, pending: resolvePending } = useAsyncAction(
    async (id: string) =>
      fetchJsonThrow(`/api/risk/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ action: "resolve" }),
      }),
    { toast: toastFn, successMessage: "Risk resolved", onSuccess: () => void fetchData() },
  );

  const { execute: handleDismiss, pending: dismissPending } = useAsyncAction(
    async (id: string) =>
      fetchJsonThrow(`/api/risk/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ action: "dismiss" }),
      }),
    { toast: toastFn, successMessage: "Risk dismissed", onSuccess: () => void fetchData() },
  );

  const actionPending = snoozePending || resolvePending || dismissPending;
                ...
                <div key={r.id} className="flex flex-wrap items-center gap-3 p-4 hover:bg-neutral-800/30">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    {(r.status === "open" || r.status === "snoozed") && (
                      <>
                        <Button variant="ghost" size="sm" onClick={() => handleSnooze(r.id, "2d")} disabled={actionPending}>
                          2d
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleSnooze(r.id, "7d")} disabled={actionPending}>
                          7d
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleResolve(r.id)} disabled={actionPending} className="text-emerald-400">
                          <Check className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDismiss(r.id)} disabled={actionPending} className="text-neutral-400" data-testid="risk-dismiss">
                          <X className="w-4 h-4" />
                        </Button>
                      </>
                    )}
