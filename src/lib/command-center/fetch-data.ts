/**
 * Shared data fetcher for the Command Center page.
 * Used by both the RSC page (server-side) and the API route (cached).
 */
...
export async function fetchCommandCenterData(): Promise<CommandCenterData> {
  ...
  const byMode: { off: number; mock: number; manual: number; live: number } = { off: 0, mock: 0, manual: 0, live: 0 };
  const errorCount = integrations.filter((i) => i.status === "error").length;
  for (const i of integrations) {
    const mode = (["off", "mock", "manual", "live"] as const).includes(i.mode as "off" | "mock" | "manual" | "live") ? (i.mode as keyof typeof byMode) : "off";
    byMode[mode] += 1;
  }
