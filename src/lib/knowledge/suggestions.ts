...
Output valid JSON only, no markdown fences: an array of objects. Each object has: title, problem, proposedChange, expectedImpact, effort ("S"|"M"|"L"), systemArea (one of: ${SYSTEM_AREAS.join(", ")}), sourceTranscriptRef (string, use the exact ref provided), status ("queued").
Generate 1-5 suggestions. Focus on highest impact. sourceTranscriptRef must be the exact string provided.`;
