...
test.describe("Client-acquisition E2E", () => {
  ...
  test("Owned audience: Log snapshot and see summary", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /save snapshot/i }).first().click();

    await expect(page.getByText(/150 subs|sends|replies|inquiries influenced/i).first()).toBeVisible({ timeout: 5000 });
  });
  ...
  test("Trust-to-close checklist: lead with proposal shows checklist and toggle saves", async ({
    ...
    // Use page.request to inherit session cookies from the logged-in page
    const artifactRes = await page.request.post(`${apiBase}/api/leads/${leadId}/artifacts`, {
    ...
    expect(artifactRes.ok(), `Artifact API returned ${artifactRes.status()}: ${await artifactRes.text()}`).toBe(true);
  ...
  test("Full flow: login → Command Center → owned-audience snapshot → networking event → lead with proposal → trust-to-close", async ({
    ...
    await page.getByRole("button", { name: /save snapshot/i }).first().click();
    await expect(page.getByText(/100 subs|sends/i).first()).toBeVisible({ timeout: 5000 });
    ...
    await page.request.post(`${apiBase}/api/leads/${leadId}/artifacts`, {
      headers: { "Content-Type": "application/json" },
